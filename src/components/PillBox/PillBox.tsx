...
export const PillBox = memo(function PillBox({
  ...
  const customStyle = !isPresetColor(color)
    ? customColorToCSSVars(color)
    : undefined;

  return (
    <span
      style={customStyle}
      className={clsx(baseClasses, scaleMap[scale], isPresetColor(color) ? colorMap[color] : "cs-custom-pill", className)}
    >
      {label}
    </span>
  );
