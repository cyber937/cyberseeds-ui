...
export function Input({
  ...
  return (
    <div>
      ...
      <input
        className={`w-full items-center rounded-md disabled:bg-amber-100 disabled:text-gray-400 font-sans outline-1 placeholder:text-gray-400 focus:outline-2 focus:-outline-offset-2 self-start ${isInvalid ? 'text-red-400 bg-red-100/50 outline-red-300' : 'text-gray-900 bg-white outline-gray-300'} ${scaleMap[scale]} ${colorMap[color]} ${className}`}
        {...props}
      />
    </div>
  )
